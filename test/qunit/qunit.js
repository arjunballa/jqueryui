...
( function(window) {
		...
		var QUnit, config, testId = 0, toString = Object.prototype.toString, hasOwn = Object.prototype.hasOwnProperty, defined = {
		...
		QUnit = {
			...
			// Specify the number of expected assertions to gurantee that failed test (no
		...
		// object exists, in that case we assume we're in CommonJS - export everything at
		// the end
		...
		// QUnit.constructor is set to the empty F() above so that we can add to it's
		// prototype later
		// Doing this allows us to tell if the following methods have been overwritten on
		// the actual
		...
		// so far supports only Firefox, Chrome and Opera (buggy), Safari (for real
		// exceptions)
					...
					return obj.__proto__;
						...
						return {
							...
							"object" : function(b, a) {
								var i, j, loop,
								// Default to true
								eq = true, aProperties = [], bProperties = [];
		...
		QUnit.jsDump = ( function() {
				...
				var reName = /^function (\w+)/, jsDump = {
					parse : function(obj, type, stack) {//type is used mostly internally, you can fix
						// a (custom)type in advance
						stack = stack || [];
					...
					parsers : {
						...
						functionCode : "[code]", //function calls it internally, it's the content of the
						// function
